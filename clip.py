...
def load_data(path_to_data, motion_sel, fraction_for_test):
    ...
    for data_root, data_dirs, data_files in os.walk(path_to_data):
        for data_file_name in data_files:
            ...
            #第一种为测试集
            if label_1 in motion_sel[1:]:  # 后五种手势用于训练
                train_data.append(data_normed_1.tolist())
                train_labels.append(label_1)
            elif label_1 == motion_sel[0]:  # 第一种手势用于测试
                test_data.append(data_normed_1.tolist())
                test_labels.append(label_1)

            #第六种为测试集
            # if label_1 in motion_sel[:5]:  # 前五种手势用于训练
            #     train_data.append(data_normed_1.tolist())
            #     train_labels.append(label_1)
            # elif label_1 == motion_sel[5]:  # 第六种手势用于测试
            #     test_data.append(data_normed_1.tolist())
            #     test_labels.append(label_1)
...
# ================== 4. 对比学习损失 ==================
# def contrastive_loss(wifi_features, text_features, temperature=0.07):
#     similarity_matrix = F.cosine_similarity(wifi_features.unsqueeze(1), text_features.unsqueeze(0), dim=2)
#     labels = torch.arange(wifi_features.size(0)).to(wifi_features.device)
#     loss = F.cross_entropy(similarity_matrix / temperature, labels)
#     return loss
# def info_nce_loss(wifi_features, text_features, temperature=0.07):
#     similarity_matrix = F.cosine_similarity(wifi_features.unsqueeze(1), text_features.unsqueeze(0), dim=2)
#     labels = torch.arange(wifi_features.size(0)).to(wifi_features.device)
#     logits = similarity_matrix / temperature
#     exp_logits = torch.exp(logits)
#     log_prob = logits[range(len(labels)), labels] - torch.log(exp_logits.sum(dim=1))
#     loss = -log_prob.mean()
#     return loss
# ================== 5. 训练函数 ==================
# def train(wifi_encoder, text_encoder, dataloader, optimizer, device, epochs=10):
#     wifi_encoder.train()
#     text_encoder.train()
#
...
def train(model, dataloader, optimizer, device, epochs=10, scheduler=None):
    model.train()
...
# ================== 6. 文本prompt转换 ==================
# def encode_class_prompts(text_encoder, tokenizer, class_labels, device):
#     '''
#
#     :param text_encoder:训练好的文本编码器
#     :param tokenizer:文本编码器的分词器
#     :param class_labels:需要转换为特征的类别列表
#     :param device:计算设备（CPU 或 GPU）
#     :return:class_features -- 提取的类别文本特征，形状为 (num_classes, feature_dim)
#     '''
#     # 生成类别对应的文本描述
#     prompts = [f"This is a motion of type {number_to_gesture20181109[label]}." for label in class_labels]
#
#     # 使用 tokenizer 处理文本
#     encoded_inputs = tokenizer(prompts, padding=True, truncation=True, max_length=128, return_tensors="pt")
#     input_ids = encoded_inputs['input_ids'].to(device)
#     attention_mask = encoded_inputs['attention_mask'].to(device)
#
#     # 获取类别文本的特征表示
#     with torch.no_grad():
#         class_features = text_encoder(input_ids, attention_mask)  # (num_classes, feature_dim)
#
#     return class_features

# ================== 6. 结果测试 ==================
# def test(wifi_encoder, text_encoder, dataloader, device, class_names):
#     wifi_encoder.eval()
#     text_encoder.eval()
#
#     # 生成所有类别的文本特征
...
#
#             # 计算与所有类别特征的相似度
#             #similarity = torch.matmul(wifi_features, class_features.T)
#             #preds = similarity.argmax(dim=1)  # 预测类别索引
#
#             # 计算与所有类别特征的相似度
#             similarity = F.cosine_similarity(wifi_features.unsqueeze(1), class_features.unsqueeze(0), dim=2)
#             preds = similarity.argmax(dim=1)  # 预测类别索引
#
#             # 将索引转换为对应的类别标签
...
#
#     target_label = 6  # 例如，你要查看第六种手势的混淆情况
#     target_index = class_labels.index(target_label)  # 找到对应的索引
#     confusion_vector = conf_matrix[target_index]  # 提取该类别的混淆向量
#     confusion_vector = confusion_vector.reshape(1, -1)  # 变为 1 行 n 列
#
#     # 按行归一化（推荐）
...
#     sns.heatmap(confusion_vector, annot=True, fmt='d', cmap='Blues', xticklabels=class_names, yticklabels=class_names)
...
def test(clip_model, dataloader, device, class_names, tokenizer):
    ...
    target_label = 1  # 例如，你要查看第六种手势的混淆情况
    target_index = class_labels.index(target_label)  # 找到对应的索引
